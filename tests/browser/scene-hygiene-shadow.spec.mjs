...
test('Suzuka scenery follows terrain and perimeter trees clear every track branch',async({page})=>{
  await boot(page);
  const r=await page.evaluate(()=>{
    ...
    const maxGroundError=groups.reduce((m,o)=>Math.max(m,Math.abs((o.position?.y??0)-(W.terrainHeightAt?.(o.position.x,o.position.z)??-.22))),0);
    ...
    return{h:W.sceneHygiene,count:trees?.count||0,minClear,maxGroundError,legacyAnonymousStand,legacyAnonymousGantry,audit:W.circuitAudit?.sceneHygiene,terrain:W.terrain?.owner};
  });
  expect(r.h?.owner,JSON.stringify(r)).toBe('runtime-scene-hygiene-v2');
  expect(r.terrain).toBe('runtime-suzuka-terrain-v1');
...
test('moving cars use live contact shadows plus a bounded near-field real shadow set',async({page})=>{
  await boot(page);
  const r=await page.evaluate(()=>{
    const W=window.__RACING_WORLD__,R=window.__RACING_RACE__,car=W.makeCar(0x3485ff,'gt'),contact=car.userData?.contactShadow;
    let casting=0,totalMeshes=0;car.traverse(o=>{if(!o?.isMesh)return;totalMeshes++;if(o!==contact&&o.castShadow)casting++;});
    const selection=W.updateHybridVehicleShadows?.(R.cars,true);let castCars=0;for(const c of R.cars){const cs=c.mesh?.userData?.contactShadow;let casts=false;c.mesh?.traverse?.(o=>{if(o?.isMesh&&o!==cs&&o.castShadow)casts=true;});if(casts)castCars++;}
    return{policy:car.userData?.vehicleShadowPolicy,base:W.vehicleShadowPolicy,hybrid:{...W.hybridShadowPolicy,selected:undefined},contact:!!contact,contactOpacity:contact?.material?.opacity??0,casting,totalMeshes,selection,castCars};
  });
  expect(r.policy,JSON.stringify(r)).toBe('hybrid-contact-only');
  expect(r.base?.owner).toBe('runtime-contact-shadow-live-v1');
  expect(r.hybrid?.owner).toBe('runtime-hybrid-vehicle-shadow-v1');
  expect(r.contact).toBeTruthy();expect(r.contactOpacity).toBeGreaterThanOrEqual(.27);expect(r.casting,JSON.stringify(r)).toBe(0);expect(r.totalMeshes).toBeGreaterThan(5);
  expect(r.castCars).toBeGreaterThan(0);expect(r.castCars).toBeLessThanOrEqual(r.hybrid.maxCasters);
});
