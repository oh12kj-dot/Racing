...
test('green-flag race develops realistic speed spread instead of a universal pace cap',async({page})=>{
  ...
  const result=await page.evaluate(()=>{
    ...
    const latest=green.at(-1)||samples.at(-1)||{speeds:[]};
    const speeds=latest.speeds||[],min=Math.min(...speeds),max=Math.max(...speeds),mean=speeds.reduce((a,b)=>a+b,0)/Math.max(1,speeds.length),variance=speeds.reduce((a,b)=>a+(b-mean)**2,0)/Math.max(1,speeds.length);
  ...
  expect(result.max,JSON.stringify(result)).toBeGreaterThan(145);
  expect(result.spread,JSON.stringify(result)).toBeGreaterThan(18);
  expect(result.std,JSON.stringify(result)).toBeGreaterThan(4);
});
