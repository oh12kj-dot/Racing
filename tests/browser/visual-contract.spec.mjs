...
test('deterministic runtime frame keeps non-blank contrast and color diversity',async({page},testInfo)=>{
  await boot(page);await page.evaluate(()=>window.__RACING_TEST_TICK__(.016,true));
  const stats=await page.evaluate(()=>{
    const W=window.__RACING_WORLD__,gl=W.renderer.getContext(),w=gl.drawingBufferWidth,h=gl.drawingBufferHeight,p=new Uint8Array(w*h*4);gl.readPixels(0,0,w,h,gl.RGBA,gl.UNSIGNED_BYTE,p);
