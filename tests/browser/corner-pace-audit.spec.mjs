...
test('isolated Formula lap exposes realistic dry Suzuka pace telemetry',async({page},testInfo)=>{
  await boot(page);
  const result=await page.evaluate(()=>{
    ...
    const c=R.cars.find(x=>x.type==='formula'&&!x.retired);
    if(!c)return{error:'NO_ACTIVE_FORMULA',types:R.cars.map(x=>x.type),total};
    ...
    Object.assign(c,{retired:false,recovered:false,pitState:'NONE',_runtimePitPhase:'TRACK',spinState:'NONE',hazardAvoiding:false,localYellow:false,hydroplaning:false,incident:0,damage:0,damageState:'NONE',fault:null,wear:0,avoid:0,blueFlag:false,coolingMode:false,predictiveSpeedCap:null,multiclassPassIntent:false,multiclassPassTargetId:null,multiclassPassLane:null,racecraftBlocked:false,battleState:'NONE',fuelKg:5});
    ...
    const mode=()=>c.racingLineMode||'OPTIMAL',curvAt=s=>Math.abs(Number(W.racingCurvatureFor?.(s,mode())??W.racingCurvatureAt?.(s)??W.curvatureAt?.(s)??0));
    const curvatureSamples=[];let maxCenterCurvature=0;for(let s=0;s<total;s+=5){const k=curvAt(s);curvatureSamples.push({s,k});maxCenterCurvature=Math.max(maxCenterCurvature,Math.abs(Number(W.curvatureAt?.(s))||0));}
    ...
    let prev=Number(c.s)||0,travel=0,start=Number(R.race.t)||0,end=null,maxSpeed=0,minSpeed=Infinity,minSpeedS=null,minSpeedK=null,sumSpeed=0,samples=0,maxLatUse=0,maxFrictionCircleUse=0,maxActualLatG=0,straightSum=0,straightN=0,curvySum=0,curvyN=0,tightSum=0,tightN=0;
    for(let i=0;i<3000&&end==null;i++){
      ...
      const v=Math.max(0,Number(c.v)||0),kmh=v*3.6,k=curvAt(s),actualLatG=v*v*k/9.81;maxSpeed=Math.max(maxSpeed,kmh);if(kmh<minSpeed){minSpeed=kmh;minSpeedS=s;minSpeedK=k;}sumSpeed+=kmh;samples++;maxLatUse=Math.max(maxLatUse,Number(c.racingLatUse)||0);maxFrictionCircleUse=Math.max(maxFrictionCircleUse,Number(c.racingFrictionCircleUse)||0);maxActualLatG=Math.max(maxActualLatG,actualLatG);
      if(k<=q25){straightSum+=kmh;straightN++;}if(k>=q75){curvySum+=kmh;curvyN++;}if(k>=q90){tightSum+=kmh;tightN++;}
      if(travel>=total)end=now;
    }
    return{type:c.type,total,lapTime:end!=null?end-start:null,maxSpeed,minSpeed,minSpeedS,minSpeedK,minRadius:minSpeedK>0?1/minSpeedK:null,maxCurvature,maxCenterCurvature,smallestRadius:maxCurvature>0?1/maxCurvature:null,meanSpeed:samples?sumSpeed/samples:0,straightMean:straightN?straightSum/straightN:0,curvyMean:curvyN?curvySum/curvyN:0,tightMean:tightN?tightSum/tightN:0,maxLatUse,maxFrictionCircleUse,maxActualLatG,q25,q75,q90,wetness:Number(W.env?.wetness)||0,lineMode:c.racingLineMode||null,completedLaps:travel/total,pitState:c.pitState,spinState:c.spinState,retired:c.retired,dynamics:R.racingDynamics?.find?.(x=>x.carId===c.id)||null};
  });
  console.log(`CORNER_PACE_AUDIT ${JSON.stringify(result)}`);
  await testInfo.attach('corner-pace-audit',{body:JSON.stringify(result,null,2),contentType:'application/json'});
  expect(result.error,JSON.stringify(result)).toBeUndefined();expect(result.type,JSON.stringify(result)).toBe('formula');
