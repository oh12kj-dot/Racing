...
async function waitForAssetDiagnostics(page){
  await page.waitForFunction(()=>{
    const a=window.__RACING_ASSETS__,r=window.__RACING_WORLD__?.renderAssets;
    return !!a?.decoderSupport||!!r?.decoderSupport||r?.state==='ready'||r?.state==='fallback';
  },null,{timeout:15000});
}
...
test('quality-preserving performance runtime is active',async({page})=>{
  ...
  const r=await page.evaluate(()=>{
    const tick=window.__RACING_TEST_TICK__;for(let i=0;i<4;i++)tick?.(.016,false);
    ...
    return{renderPath:W.runtimeRenderPath,optimizer:o,gridSize:W.runtimeSpatialNeighbours?.size||0,ui:W.runtimeUIBudget,strategy:s,collision:c,pit:p,shadow:{...W.hybridShadowPolicy,selected:undefined},renderAssets:W.renderAssets?{state:W.renderAssets.state,decoderSupport:W.renderAssets.decoderSupport||window.__RACING_ASSETS__?.decoderSupport}:null};
  });
