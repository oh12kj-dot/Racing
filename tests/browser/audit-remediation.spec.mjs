...
test('pit state machine resolves circuit-owned layout into a generic runtime spec',async({page})=>{
  ...
  expect(x.mergeLaneTarget).toBeGreaterThan(3);expect(x.mergeLaneTarget).toBeLessThanOrEqual(3.55);
...
test('pit runtime spec preserves safe defaults without optional exit metadata',async({page})=>{
  ...
  expect(x.exitEndUF).toBeCloseTo(1.05,6);expect(x.mergeTrackOffset).toBeCloseTo(4,6);expect(x.mergeLaneTarget).toBeCloseTo(2.5,6);
});
