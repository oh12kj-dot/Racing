...
(async()=>{try{
 ...
 function frame(dt,now=performance.now(),render=true){
   ...
   let shadowDid=false;const shadowStep=PM.interval('shadow');if(!runtimeTest&&Number.isFinite(shadowStep)){shadowAcc+=dt;if(shadowAcc>=shadowStep){W.renderer.shadowMap.needsUpdate=true;shadowAcc=0;shadowDid=true;}}
   if(!runtimeTest)P.measure('AUDIO',()=>{A.setFocus(idx);A.update(dt);});P.measure('UI',()=>U.update(dt,idx));
   if(render){const q=P.gpuBegin();P.measure('RENDER CPU',()=>W.renderer.render(W.scene,W.camera));P.gpuEnd(q);if(!mobile&&!runtimeTest)P.measure('PIP',()=>B.render());}
