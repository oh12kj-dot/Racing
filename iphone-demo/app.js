...
(async()=>{try{
 ...
 const W=buildWorld(THREE,TRACK),R=createRace(W,statusEl),D=createDirector(R),E=createEnvironment(W,R),C=createCamera(W,R,D,camEl),U=createUI(W,R,D,E,C);
 ...
 const clock=new THREE.Clock();W.renderer.setAnimationLoop(()=>{const dt=Math.min(.05,clock.getDelta());R.update(dt);D.update(dt);E.update(dt);const idx=C.update(dt,performance.now()),c=R.cars[idx]||R.getStandings()[0];U.update(dt,idx);W.renderer.render(W.scene,W.camera);if(c)speedEl.textContent=`CAR ${c.number} · ${c.name} · ${c.type.toUpperCase()} · ${Math.round(c.v*3.6)} km/h`;});
}catch(e){fail(e)}})();
