...
import {createDirector} from './v18-director.js';
...
import {createUI} from './v18-ui.js';
...
(async()=>{try{
 ...
 const W=buildWorld(THREE,track,settings,circuit.name),R=createRace(W,statusEl,settings),D=createDirector(R),E=createEnvironment(W,R,settings),C=createCamera(W,R,D,camEl),A=createAudio(R),U=createUI(W,R,D,E,C,A,settings,saveSettings),S=createSafetyCar(W,R),B=createBroadcast(W,R,D);
 ...
 W.renderer.setAnimationLoop(()=>{const dt=Math.min(.05,clock.getDelta());R.update(dt);if(['RED','VSC','SC'].includes(R.flag))statusEl.textContent=R.flag;D.update(dt);E.update(dt);S.update();const idx=C.update(dt,performance.now()),c=R.cars[idx]||R.getStandings()[0];W.updateVehicleLOD?.(R.cars);A.setFocus(idx);A.update(dt);U.update(dt,idx);W.renderer.render(W.scene,W.camera);B.render();if(c){const flags=[c.blueFlag?'BLUE':'',c.hydroplaning?'HYDRO':'',c._doubleStackWait>0?'DOUBLE STACK':'',c.drsActive?'DRS':'',c.coolingMode?'COOL':'',c.spinState&&c.spinState!=='NONE'?c.spinState:''].filter(Boolean).join(' · ');speedEl.textContent=`${W.circuitName} · ${R.raceClass||'MIXED'} · CAR ${c.number} ${c.name} · ${c.driverStyle||''} · P${c.position} · ${Math.round(c.v*3.6)} km/h${flags?' · '+flags:''}`;}});
}catch(e){fail(e)}})();
