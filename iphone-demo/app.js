...
(async()=>{try{
 ...
 const D=createDirector(R),E=createEnvironment(W,R,settings),F=createEnvironmentReflections(W,{mobile}),C=createCamera(W,R,D,camEl),A=createAudio(R,settings),Q=createRuntimeRegression(W,R),QL=createSceneQualityController(W,{mobile}),G=createSelectiveGlow(W,R.cars,{mobile}),VM=createVehicleMotion(W,R,{mobile});W.updateVisualWeather?.();QL.apply(runtimeTest?3:PM.level,R.cars);G.update?.(runtimeTest?3:PM.level);attachRuntimeAudit(W,{race:R,performance:PM,regression:Q,quality:QL,reflections:F});
 try{window.__RACING_RACE__=R;window.__RACING_DIRECTOR__=D;window.__RACING_AUDIO__=A;window.__RACING_ASSETS__=AM;window.__RACING_ASSET_LOAD__=assetLoad;window.__RACING_REGRESSION_MONITOR__=Q;window.__RACING_REFLECTIONS__=F;window.__RACING_SCENE_QUALITY__=QL;window.__RACING_GLOW__=G;window.__RACING_VEHICLE_MOTION__=VM;window.__RACING_AUDIT__=W.auditCircuit?.();}catch{}
 ...
 function updateSessionHUD(){const st=R.getStandings(),leader=st[0],phase=R.sessionPhase;if(phase==='QUALIFYING'){const q=R.qualifying?.[0],pole=q?R.cars[q.carId]:null;statusEl.textContent=`QUALIFYING${pole?` · P1 ${pole.name}`:''}`;return;}if(phase==='FORMATION'){statusEl.textContent='FORMATION LAP';return;}const classification=R.classification,complete=!!classification?.complete,winnerFinished=classification?.winnerFinishAt!=null,lap=Math.min(R.race.lapsTarget,Math.max(1,(leader?.lap??0)+1));if(complete){statusEl.textContent='FINISH · RACE COMPLETE';return;}if(winnerFinished){const finished=classification?.finishers?.length||0,remaining=R.cars.filter(c=>!c.retired&&!c.finished).length;statusEl.textContent=`FINISH · FIELD COMPLETING · ${finished} FINISHED · ${remaining} RUNNING`;return;}statusEl.textContent=`RACE · LAP ${lap}/${R.race.lapsTarget} · ${R.flag||'GREEN'}`;}
