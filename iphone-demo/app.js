...
(async()=>{try{
 ...
 W.renderer.setAnimationLoop(()=>{
   ...
   lodAcc+=dt;const lodStep=PM.interval('lod');if(lodAcc>=lodStep){P.measure('LOD',()=>{W.updateVehicleLOD?.(R.cars);W.updateShadowVisibility?.(R.cars);QL.apply(PM.level,R.cars);G.update?.(PM.level);});lodAcc=0;}
