...
export function createCameraDirector(world,track){
  ...
  function update(snapshot){
    ...
    else if(use==='ONBOARD'){pos=carPos.clone().add(new THREE.Vector3(0,1.35,0)).addScaledVector(f,.7);target=carPos.clone().addScaledVector(f,28).add(new THREE.Vector3(0,.65,0));}
    ...
    camera.position.lerp(pos,.09);camera.lookAt(target);return{mode:use,tracked:car};
  }
