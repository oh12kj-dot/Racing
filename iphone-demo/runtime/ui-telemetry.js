export function attachTelemetryUI(base,W,R){
  ...
  return new Proxy(base,{get(target,prop){if(prop==='update')return(dt,id)=>{focusId=Number(id)||0;oldUpdate(dt,id);acc+=dt;if(active&&acc>=.5){acc=0;render();}};return Reflect.get(target,prop,target);}});
}
