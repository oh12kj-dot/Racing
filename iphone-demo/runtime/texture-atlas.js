export function atlasRuntimeCanvasTextures(W){
  ...
  root.traverse?.(o=>{
    if(!o?.isMesh||Array.isArray(o.material)||!o.material?.map?.isCanvasTexture||!o.material.map.image)return;
    const image=o.material.map.image,w=Number(image.width)||0,h=Number(image.height)||0;if(!w||!h)return;
    const key=`${o.material.type}|${o.material.side}|${w}x${h}`;let a=groups.get(key);if(!a)groups.set(key,a=[]);a.push(o);
  });
  ...
  for(const list of groups.values()){
    ...
    for(let i=0;i<list.length;i++){const col=i%cols,row=Math.floor(i/cols);g.drawImage(list[i].material.map.image,col*tileW,row*tileH,tileW,tileH);}
    const tex=new T.CanvasTexture(canvas);tex.colorSpace=first.material.map.colorSpace;tex.wrapS=first.material.map.wrapS;tex.wrapT=first.material.map.wrapT;tex.magFilter=first.material.map.magFilter;tex.minFilter=first.material.map.minFilter;tex.anisotropy=first.material.map.anisotropy;tex.needsUpdate=true;
    const mat=first.material.clone();mat.map=tex;mat.needsUpdate=true;
    for(let i=0;i<list.length;i++){
      const mesh=list[i],oldGeo=mesh.geometry,oldMat=mesh.material,oldMap=oldMat.map,col=i%cols,row=Math.floor(i/cols),geo=new T.BufferGeometry().copy(oldGeo),uv=geo.attributes.uv;if(!uv)continue;const arr=new Float32Array(uv.array.length);for(let j=0;j<uv.count;j++){arr[j*2]=(col+uv.getX(j))/cols;arr[j*2+1]=(rows-1-row+uv.getY(j))/rows;}geo.setAttribute('uv',new T.BufferAttribute(arr,2));geo.userData.runtimeAtlas=true;mesh.geometry=geo;mesh.material=mat;oldGeo?.dispose?.();oldMap?.dispose?.();oldMat?.dispose?.();texturesBefore++;meshes++;
    }
