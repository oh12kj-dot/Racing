...
export function createRace(W,statusEl,settings={}){
  ...
  function update(dt){
    reserveSideBySide();const eventStart=R.events?.length||0;baseUpdate(dt);
  ...
  return new Proxy(R,{get(target,prop){if(prop==='update')return update;if(prop==='sideBySideControl')return{activeReservations:reservations.size,recentRubs:[...rubs],contactPolicy:'physical-contact-authoritative'};return Reflect.get(target,prop,target);}});
}
