...
export function buildWorld(THREE,TRACK,settings={},circuitName='SUZUKA'){
  ...
  const baseMake=W.makeCar.bind(W),bodyFitTypes=new Set(['gt','touring','supercar']);
  W.makeCar=(color,type)=>{
    const car=baseMake(color,type),livery=car.userData?.livery;
    if(bodyFitTypes.has(type)&&livery?.group){
      // Planar sponsor/sweep geometry cannot conform reliably to every closed-car
      // procedural body or imported GLB. Keep the native paint/accent/carbon
      // panels, but suppress free-floating graphics so GT, touring and supercar
      // liveries cannot protrude through doors, fenders or shoulder surfaces.
  ...
  W.vehicleSizeAudit={owner:'runtime-real-scale-v1',targets:W.vehicleDimensionTargets||null,gt:W.vehicleDimensionTargets?.gt||null,touring:W.vehicleDimensionTargets?.touring||null,supercar:W.vehicleDimensionTargets?.supercar||null};
