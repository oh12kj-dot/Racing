export function createBarrierSafety(W,R){
  ...
  function resolveCar(c){
    ...
    if(residual){
      ...
      c.laneTarget=clamp((c.laneTarget||0)+latPush,-4.15,4.15);c.v=Math.min(c.v||0,13);metrics.deepCorrections++;metrics.lastCarId=c.id;totalPush+=push;residual=W.barrierContact(c);
    }
    ...
    for(let i=0;i<3;i++){
      residual=W.barrierContact(c);if(!residual)break;const push=clamp((Number(residual.penetration)||0)+.24,.24,1.7);c.mesh.position.addScaledVector(residual.inward,push);totalPush+=push;metrics.corrections++;
    }
