...
export function createRace(W,statusEl,settings={}){
  const R=createV12Race(W,statusEl,settings),baseUpdate=R.update;
  ...
  function tyre(c,dt){const old=c._pitCompatWearPrev??c.wear??0,delta=Math.max(0,(c.wear||0)-old);c._pitCompatWearPrev=c.wear;c._strategyWearRate=c._strategyWearRate*.985+delta/Math.max(dt,.001)*80*.015;if(c.pitState==='STOP'&&(c.wear||0)<.08)c.flatSpot=false;W.updateTyreVisual?.(c);}
