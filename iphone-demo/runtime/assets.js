export function createRenderAssetManager(W,{mobile=false}={}){
  const manifestUrl=new URL('../../assets/render/manifest.json',import.meta.url),cache=new Map(),failures=[];let manifest=null,loaderPromise=null;
  ...
  function prepareScene(scene){scene.traverse(o=>{if(!o.isMesh)return;o.castShadow=true;o.receiveShadow=true;const mats=Array.isArray(o.material)?o.material:[o.material];for(const m of mats){if(!m)continue;if('envMapIntensity'in m)m.envMapIntensity=Math.max(.85,m.envMapIntensity||0);}});}
  async function upgradeCar(car,entry){
    try{
      const gltf=await loadModel(entry.url),scene=gltf.scene.clone(true);prepareScene(scene);
      const visual=car.mesh?.userData?.visual;if(visual)visual.visible=false;
      const scale=Number(entry.scale)||1,rot=entry.rotation||[0,0,0],off=entry.offset||[0,0,0];scene.scale.setScalar(scale);scene.rotation.set(Number(rot[0])||0,Number(rot[1])||0,Number(rot[2])||0);scene.position.set(Number(off[0])||0,Number(off[1])||0,Number(off[2])||0);scene.name=`GLB_${car.type||'CAR'}`;car.mesh.add(scene);car.mesh.userData.renderAsset=scene;return true;
  ...
  async function upgradeCars(cars=[]){
    ...
    const result=await Promise.all(jobs);W.renderAssets={manifestVersion:m?.version||0,requested:jobs.length,loaded:result.filter(Boolean).length,failed:failures.length,mobile};return W.renderAssets;
  }
