...
export function createRace(W,statusEl,settings={}){
  ...
  function speedEnvelope(c,mode){
    ...
    c.racingDamagePerformance=damagePerf;c.racingLineDeviation=deviation;return{target:Math.max(8,target),brakeBase,gCap,damagePerf};
  }
  function longitudinal(c,dt,before){
    ...
    const spec=performanceFor(c.type),k=Math.abs(lineCurv(mode,c.s)),latDemand=before*before*k,latUse=clamp(latDemand/Math.max(1,x.gCap),0,.985),longAvail=Math.sqrt(Math.max(.03,1-latUse*latUse)),accelBase=Math.max(2.5,c._v18BaseAccel||c.accel||spec.accel)*(1+(c.energyMode==='PUSH'?.07:0))*(.45+.55*longAvail)*(x.damagePerf?.accelerationScale??1),brakeAvail=x.brakeBase*(.30+.70*longAvail);
