  ...
  function predictiveAvoidance(dt){
    ...
    for(const c of R.cars){
      ...
      for(const a of threats){
        ...
        if(conflict&&follow.shouldCap&&follow.ttc<follow.ttcLimit){const ttcAllowance=Math.max(0,follow.allowedSpeed-(front.v||0));requestSpeedCap(c,front.v+ttcAllowance);if(!follow.passEscape||a.dist<bodyGap+4)c.avoid=Math.max(c.avoid||0,.65);}
      }
