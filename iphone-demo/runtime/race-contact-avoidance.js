...
export function createRace(W,statusEl,settings={}){
  ...
  function nearestAhead(c){const spatial=spatialFor(c),candidate=spatial?.aheadView||spatial?.ahead;if(candidate?.car)return candidate;let car=null,dist=Infinity;for(const o of R.cars){if(o===c||o.retired||o.pitState!=='NONE')continue;const d=forwardGap(c,o);if(d>0&&d<dist){dist=d;car=o;}}return{car,dist};}
  ...
  return new Proxy(R,{get(target,prop){if(prop==='update')return filteredUpdate;if(prop==='collisionAvoidance')return{raceStartAt,suppressedFalseContacts:suppressed.length,lateralVetoes,spatialGrid:!!(R.spatialNeighbours||W.runtimeSpatialNeighbours),snapshotPool:snapshots.length};return Reflect.get(target,prop,target);}});
}
