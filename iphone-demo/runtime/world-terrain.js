...
export function buildWorld(THREE,TRACK,settings={},circuitName='SUZUKA'){
  ...
  const samples=[];
  for(let i=0;i<720;i++){
    const q=W.sample(total*i/720);samples.push({x:q.p.x,y:q.p.y,z:q.p.z});
  }
  ...
  function nearestTrack(x,z){
    ...
    for(const p of samples){const dx=x-p.x,dz=z-p.z,d=dx*dx+dz*dz;if(d<best){best=d;bestY=p.y;}}
  ...
  function terrainHeightAt(x,z){
    const {dist,bestY}=nearestTrack(x,z);
    if(dist>=240)return BASE_Y;
    const influence=1-smooth((dist-16)/(240-16));
    // Keep the terrain comfortably below every driveable ribbon near the circuit.
    // The dedicated high-resolution grass/gravel/road strips own the first 31 m
    // from the centreline, so the coarse venue terrain must never poke through.
    const clearance=dist<34?.58:.20;
    return BASE_Y+(bestY-clearance-BASE_Y)*influence;
  }
  ...
  const nx=144,nz=96,pos=[],uv=[],ind=[],vertexTrackDistance=new Float32Array((nx+1)*(nz+1));
  for(let iz=0;iz<=nz;iz++)for(let ix=0;ix<=nx;ix++){
    const u=ix/nx,v=iz/nz,x=cx-width/2+u*width,z=cz-depth/2+v*depth,near=nearestTrack(x,z),y=terrainHeightAt(x,z),vi=iz*(nx+1)+ix;
    vertexTrackDistance[vi]=near.dist;pos.push(x,y,z);uv.push(u*34,v*24);
  }
  const CUTOUT_RADIUS=27.5;
  for(let iz=0;iz<nz;iz++)for(let ix=0;ix<nx;ix++){
    const a=iz*(nx+1)+ix,b=a+1,c=(iz+1)*(nx+1)+ix,d=c+1;
    // Leave a continuous hole beneath the smooth 31 m trackside grass strip.
    // This removes the triangulated terrain from the road/pit shoulder entirely,
    // eliminating jagged green triangles that could intersect the asphalt.
    if(Math.min(vertexTrackDistance[a],vertexTrackDistance[b],vertexTrackDistance[c],vertexTrackDistance[d])<CUTOUT_RADIUS)continue;
  ...
  W.terrain={owner:'runtime-suzuka-terrain-v2',mesh:terrain,baseY:BASE_Y,width,depth,segmentsX:nx,segmentsZ:nz,minY,maxY,replacedFlatGround:!!legacyGround,figureEightLowerBranch:true,trackCutoutRadius:CUTOUT_RADIUS,trackClearance:.58};
