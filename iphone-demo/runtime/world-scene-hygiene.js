...
// owns all race/pit physics; this pass only removes obsolete render objects,
// grounds remote scenery on the actual flat terrain plane used by the demo and
// rebuilds the perimeter tree belt with a whole-circuit clearance test.
export function buildWorld(THREE,TRACK,settings={},circuitName='SUZUKA'){
  ...
  const scene=W.scene,total=Math.max(1,Number(W.total)||1),GROUND_Y=-.18;
  ...
  // Remove the very first procedural tree set completely. A later Suzuka-specific
  // belt replaces it, so keeping both only creates duplicate vegetation and makes
  // it possible for an old tree to survive near a different branch of the figure-8.
  ...
  // The base v9 scene also carried anonymous generic grandstands and a generic
  // start gantry. They pre-date the dedicated Suzuka layers and now only overlap
  // the venue-specific objects, so remove those signature-matched scene roots.
  ...
  // The demo terrain is a flat plane at roughly -0.22 m. world-suzuka-full was
  // intentionally anchored to track elevation; at the overpass that copied an
  // elevated road Y coordinate tens of metres sideways and made remote objects
  // visibly float. Ground every off-track full-scene group except actual bridges.
  const fullRoot=W.suzukaFullScene?.root;
  if(fullRoot){
    for(const o of fullRoot.children){
      if(!o?.isGroup||String(o.name||'').includes('BRIDGE'))continue;
      if(Math.abs((o.position?.y??GROUND_Y)-GROUND_Y)>.02){o.position.y=GROUND_Y;groundedObjects++;}
    }
  }
  // Do the same for the main-straight facility layer only when the object lives
  // well away from the racing surface. Race equipment stays aligned to the track.
  ...
  // Remove the first full-scene perimeter instances; they only checked distance
  // from their source segment. On Suzuka's figure-8 that can still put a tree on
  // a completely different section of track.
  ...
  // Avoid major venue footprints as well as the racing corridor. This prevents
  // the rebuilt forest from cutting through stands, hotels, fan zones or paddock.
  ...
  const targetTrees=168,maxCandidates=1800,trunkGeo=new THREE.CylinderGeometry(.16,.24,2.4,5),crownGeo=new THREE.ConeGeometry(1.75,4.7,6);
  ...
  for(let i=0;i<maxCandidates&&safeCount<targetTrees;i++){
    ...
    const scale=.76+((i*37)%31)/100,idx=safeCount++;
    minClearance=Math.min(minClearance,Math.sqrt(dSq));
    dummy.position.set(q.p.x,GROUND_Y+1.2*scale,q.p.z);dummy.rotation.set(0,(i*2.399)%6.28,0);dummy.scale.set(scale,scale,scale);dummy.updateMatrix();trunks.setMatrixAt(idx,dummy.matrix);
    dummy.position.set(q.p.x,GROUND_Y+4.05*scale,q.p.z);dummy.rotation.set(0,(i*1.731)%6.28,0);dummy.scale.set(scale,scale,scale);dummy.updateMatrix();crowns.setMatrixAt(idx,dummy.matrix);
  }
  ...
  W.sceneHygiene={
    owner:'runtime-scene-hygiene-v1',groundY:GROUND_Y,wholeCircuitTreeClearance:true,minTreeTrackClearance:Number.isFinite(minClearance)?minClearance:null,
  ...
  W.auditCircuit=()=>{
    ...
    return{...a,version:'runtime-2026.09.15-r14',suzukaFullScene:full,sceneHygiene:{...W.sceneHygiene},notes:[...(a.notes||[]),'Scene hygiene grounds remote Suzuka scenery, removes obsolete generic stands/gantry/trees, and enforces whole-circuit clearance for the perimeter tree belt.']};
  };
