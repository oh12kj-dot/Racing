...
export function createPitStateMachine(W,R){
  ...
  let arrivalSerial=0;
  const metrics={queued:0,released:0,services:0,legacyStopsRejected:0,doubleStacks:0,invariantRepairs:0,releaseWaits:0,safeReleases:0};
  ...
  function ensureArrival(c){
    ...
    if(c.pitState==='NONE'&&c._runtimePitArrival!=null&&c._runtimePitPhase!=='MERGE'){
      c._runtimePitArrival=null;c._runtimePitQueued=false;c._runtimeQueueS=null;c._runtimeReleaseWait=false;c._runtimePitPhase='TRACK';
    }
  }
  ...
  function fastLaneTraffic(c){
    ...
    for(const o of R.cars){
      ...
      const fast=phase==='FAST_LANE'||phase==='FAST_LANE_EXIT'||phase==='PIT_ENTRY'||o.pitState==='EXIT';
      if(!fast||Math.max(0,o.v||0)<1)continue;
      const delta=pitMeters(o.s)-u;
      if(delta>-RELEASE_BEHIND_METERS&&delta<RELEASE_AHEAD_METERS)near.push({car:o,delta});
    }
  ...
  function startService(c){
    const boxS=W.pitBoxS?.(c.teamId);if(!Number.isFinite(boxS))return false;
    c._runtimePitQueued=false;c._runtimeQueueS=null;c._runtimeReleaseWait=false;c._runtimePitPhase='SERVICE';c.s=boxS;c.v=0;c.pitState='STOP';c.pitTimer=serviceTime[c.type]||3.5;c._pitStopInitial=c.pitTimer;c.pitLaneStatus='JACKS';
  ...
  function holdRelease(c,count=true){
    ...
    if(count&&!c._runtimeReleaseWait)metrics.releaseWaits++;
    c._runtimeReleaseWait=true;c._runtimePitPhase='RELEASE_WAIT';c.s=boxS;c.v=0;c.pitState='EXIT';c.pitTimer=0;c.pitLaneStatus='RELEASE WAIT';posePit(c,'EXIT');
  }
  function releaseToFastLane(c){
    c._runtimeReleaseWait=false;c._runtimePitPhase='WORKING_EXIT';c.pitState='EXIT';c.v=Math.max(c.v||0,7);c.pitLaneStatus='RELEASE';posePit(c,'EXIT');metrics.safeReleases++;
  }
  ...
  function beforeUpdate(dt){
    ...
    for(const c of R.cars){
      ...
      snapshot[c.id]={state:c.pitState,v:c.v||0,s:c.s||0,phase:c._runtimePitPhase||'TRACK',releaseWait:!!c._runtimeReleaseWait};
      if(c.pitState==='ENTRY'){
  ...
  function afterUpdate(dt,snapshot,eventStart){
    ...
    for(const c of R.cars){
      ...
      if(b.state==='STOP'&&c.pitState==='EXIT'&&!c.retired&&releaseBlocked(c)){
        holdRelease(c);repairInvariant(c);continue;
      }
      if(b.releaseWait&&releaseBlocked(c)){
        holdRelease(c,false);repairInvariant(c);continue;
      }
      if(c.pitState==='STOP'){
        c._runtimePitPhase='SERVICE';c.s=W.pitBoxS?.(c.teamId)??c.s;c.v=0;c.pitLaneStatus='JACKS';posePit(c,'STOP');
      }else if(c.pitState==='EXIT'){
        if(c._runtimeReleaseWait){holdRelease(c,false);}else{c._runtimePitPhase='FAST_LANE_EXIT';c.pitLaneStatus='PIT EXIT';posePit(c,'EXIT');}
        ...
        c._runtimePitPhase='TRACK';c._runtimePitArrival=null;c._runtimePitQueued=false;c._runtimeQueueS=null;c._runtimeReleaseWait=false;c.pitLaneStatus='TRACK';
      }
      repairInvariant(c);
    }
  }
  ...
  function diagnostics(){
    return{owner:'runtime',metrics:{...metrics},cars:R.cars.filter(c=>c.pitState!=='NONE'||c._runtimePitPhase==='MERGE').map(c=>({id:c.id,team:c.teamId,state:c.pitState,phase:c._runtimePitPhase||'TRACK',queued:!!c._runtimePitQueued,releaseWait:!!c._runtimeReleaseWait,s:c.s,v:c.v,status:c.pitLaneStatus}))};
  }
