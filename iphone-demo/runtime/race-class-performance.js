...
export function createRace(W,statusEl,settings={}){
  ...
  function tune(c){const p=performanceFor(c.type),longitudinal=longitudinalPerformance(c.type,c.v||0),m=c.machine||{},load=clamp(W.braking?.(c.s)||0,0,1),wet=clamp(W.env?.wetness||0,0,1),wetScale=1-(1-p.wet)*wet,power=clamp(Number(m.power)||1,.90,1.10),aero=clamp(Number(m.aero)||1,.90,1.10),brake=clamp(Number(m.brake)||1,.90,1.10);c.classPerformance={...p,massKg:p.mass};c.massKg=p.mass;c.paceIndex=p.paceIndex;c.baseMaxNominal=p.top;c.accelNominal=longitudinal.accel;c.brakeNominal=longitudinal.brake;c._v18BaseMax=p.top*(.992+.008*power)*(.99+.01*aero)*wetScale;c._v18BaseAccel=longitudinal.accel*power*(1-load*.10*(c.type==='formula'?.35:1));c._v18BaseBrake=longitudinal.brake*brake;}
