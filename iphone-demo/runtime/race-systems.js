...
export function createRace(W,statusEl,settings={}){
  ...
  const hybridTypes=new Set(['formula','hyper','lmh','proto']),isHybrid=t=>hybridTypes.has(t);
  const wheelNames=['FL','FR','RL','RR'],stewardCases=[],neighbourCache=new Map(),activeCars=[],prevPosById=[],prevPit=[];
  ...
  for(const c of R.cars){
    ...
    c.driverState={confidence:.72,morale:.72,pressure:.28,fatigue:0};c._v24PitFailure=null;
    neighbourCache.set(c.id,{aheadView:{car:null,dist:Infinity},behindView:{car:null,dist:Infinity}});
  }

  function refreshNeighbours(){
    standingsCache=R.getStandings();activeCars.length=0;for(const c of R.cars)if(!c.retired)activeCars.push(c);activeCars.sort((a,b)=>(a.s||0)-(b.s||0));const n=activeCars.length;if(!n)return;
    for(let i=0;i<n;i++){
      const c=activeCars[i],a=activeCars[(i+1)%n],b=activeCars[(i-1+n)%n],rec=neighbourCache.get(c.id),av=rec.aheadView,bv=rec.behindView;
      av.car=a===c?null:a;av.dist=a===c?Infinity:wrap((a.s||0)-(c.s||0));bv.car=b===c?null:b;bv.dist=b===c?Infinity:wrap((c.s||0)-(b.s||0));
    }
  }
  const emptyAhead={car:null,dist:Infinity},emptyBehind={car:null,dist:Infinity};
  function aheadOf(c){return neighbourCache.get(c.id)?.aheadView||emptyAhead;}
  function behindOf(c){return neighbourCache.get(c.id)?.behindView||emptyBehind;}

  function strategyUpdate(){
    const st=standingsCache.length?standingsCache:R.getStandings(),wet=W.env?.wetness||0,flag=R.flag;
    for(const c of st){
      if(c.retired)continue;const a=aheadOf(c),b=behindOf(c),lapsLeft=Math.max(0,R.race.lapsTarget-(c.lap||0));
      const tyreLife=Math.max(.2,(1-(c.wear||0))/Math.max(.01,.018+(c.driverFatigue||0)*.004));
      const cheap=flag==='SC'||flag==='VSC',needWet=wet>.44&&c.compound!=='WET',needDry=wet<.18&&c.compound==='WET',worn=(c.wear||0)>.70;
      const gapAhead=a.car?a.dist/Math.max(10,c.v):99,gapBehind=b.car?b.dist/Math.max(10,b.car.v):99;
      const undercut=gapAhead<3.2&&gapBehind>2.2&&c.wear>.48?clamp((3.2-gapAhead)/3.2,0,1):0,overcut=gapAhead<4&&c.wear<.48?clamp((.5-c.wear)*1.8,0,1):0;
  ...
  function energyAndTraffic(c,dt){
    ...
    if(a.car){const lat=Math.abs((a.car.lane||0)-(c.lane||0));if(a.dist<32&&lat<2.4){slip=clamp((32-a.dist)/32,0,1)*straight;dirty=clamp((24-a.dist)/24,0,1)*load*(c.type==='formula'?1:hybrid?.65:.32);}}
    c.slipstream=slip;c.dirtyAir=dirty;const mode=c.energyMode==='PUSH'?1:c.energyMode==='SAVE'?-1:0;
    ...
    c.liftCoast=c.energyMode==='SAVE'?clamp(.14+Math.max(0,.25-c.fuel),.14,.30):0;const accelGain=slip*.42+(hybrid?Math.min(c.ers,.35)*(mode>0?.48:.18):0),dragLoss=dirty*(c.type==='formula'?1.15:.55),saveLoss=c.liftCoast*.9;c.v=clamp(c.v+(accelGain-dragLoss-saveLoss)*dt,0,(c.classPerformance?.top||90)*(1+slip*.025));
  }
  ...
  function wheelPhysics(c,dt){
    ...
    for(let i=0;i<4;i++){const w=c.wheelState[i],front=i<2,left=i%2===0,isOuter=turn===0?false:(turn>0?left:!left),outer=turn===0?0:(isOuter?1:-1),target=(c.tyreTemp||86)+(front?brake*8:brake*3)+outer*4+load*4-wet*3;w.temp+=(target-w.temp)*(1-Math.exp(-.8*dt));w.brake+=((c.brakeTemp||320)+(front?80:25)*brake-w.brake)*(1-Math.exp(-1.1*dt));w.wear=clamp(Math.max(w.wear,c.wear||0)+(outer>0?load*.00008:0),0,1);if(c.spinState==='LOCKUP'&&front)w.flat=clamp(w.flat+dt*.08,0,1);if(c.flatSpot)w.flat=Math.max(w.flat,.25);if(w.puncture)c.v=Math.min(c.v,18);}
    let wear=0;for(const w of c.wheelState)wear+=w.wear;c.wear=Math.max(c.wear||0,wear*.25);
  }
  ...
  function yellowOvertakes(){if(R.flag!=='YELLOW'&&R.flag!=='SC'&&R.flag!=='VSC')return;for(const c of R.cars){const p=prevPosById[c.id];if(Number.isFinite(p)&&c.position<p&&!c.retired){const x=stewardCases.find(s=>s.type==='YELLOW_OVERTAKE'&&s.carId===c.id&&s.status!=='CLOSED');if(!x)addCase('YELLOW_OVERTAKE',c,null,.45);}}}
  function update(dt){for(let i=0;i<R.cars.length;i++){const c=R.cars[i];prevPosById[c.id]=c.position;prevPit[i]=c.pitState;}baseUpdate(dt);refreshNeighbours();strategyClock+=dt;driverClock+=dt;if(strategyClock>=1.2){strategyClock=0;strategyUpdate();}for(let i=0;i<R.cars.length;i++){const c=R.cars[i];energyAndTraffic(c,dt);wheelPhysics(c,dt);trackEvolution(c,dt);pitFailures(c,prevPit[i]);if(driverClock>=.5)driverDynamics(c,.5);}if(driverClock>=.5)driverClock=0;yellowOvertakes();stewardUpdate(dt);restartRules(dt);}
  refreshNeighbours();
