export function createAudio(R){
  ...
  function speakImmediate(msg,{cancel=false}={}){const s=synth();if(!s||!enabled)return false;if(cancel)s.cancel();ctx?.resume?.();const u=makeUtterance(msg);speaking=true;gameBus?.gain.setTargetAtTime(.34,ctx.currentTime,.045);ptt(true);u.onstart=()=>{speechUnlocked=true;};u.onend=finishTransmission;u.onerror=finishTransmission;setTimeout(()=>s.speak(u),145);return true;}
