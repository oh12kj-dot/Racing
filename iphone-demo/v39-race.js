...
export function createRace(W,statusEl,settings={}){
  ...
  function applyLine(c,dt){
    ...
    const strength=clamp(Math.abs(W.curvatureAt?.(c.s+55)||0)*55+Math.abs(W.curvatureAt?.(c.s)||0)*25,0,1),rate=1.55+strength*2.25,f=1-Math.exp(-rate*dt);
    c.laneTarget+=(ideal-c.laneTarget)*f;
  }

  function speedEnvelope(c){
    const top=Math.max(24,c.classPerformance?.top||c.baseMaxNominal||c.max||72),wet=clamp(W.env?.wetness||0,0,1),wear=clamp(c.wear||0,0,1),tempGrip=clamp(c.tempGrip||1,.75,1.08),surface=clamp(c.surfaceGrip||1,.80,1.08),aero=clamp(Math.min(c.aeroFront??1,c.aeroRear??1),.55,1),tyreGrip=clamp(tempGrip*surface*(1-wear*.13)*(1-wet*.20),.58,1.06),g=(latG[c.type]||2.0)*9.81*tyreGrip*(.82+.18*aero),brakeBase=Math.max(7,c._v18BaseBrake||c.brake||15.5),brake=brakeBase*clamp(.74+.26*tyreGrip,.62,1.04)*(1-wet*.20);let target=top;
    for(const d of look){const k=Math.abs(W.curvatureAt?.(c.s+d)||0);if(k<.00105)continue;const corner=Math.min(top,Math.sqrt(Math.max(1,g/k))),allowed=Math.sqrt(corner*corner+2*brake*d);if(allowed<target)target=allowed;}
  ...
  function update(dt){
    ...
    baseUpdate(dt);
    for(const c of R.cars)postSpeed(c,dt);
  }

  return new Proxy(R,{get(target,prop){if(prop==='update')return update;if(prop==='racingDynamics')return R.cars.map(c=>({carId:c.id,line:c.racingLineTarget??0,targetSpeed:c.racingSpeedTarget??0,brake:c.racingBrake??0,throttle:c.racingThrottle??0}));return Reflect.get(target,prop,target);}});
}
