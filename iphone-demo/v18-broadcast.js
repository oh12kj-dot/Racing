export function createBroadcast(W,R,D){
  ...
  function render(){
    ...
    const renderer=W.renderer,pr=renderer.getPixelRatio?.()||1;renderer.getDrawingBufferSize(size);const w=Math.floor(Math.min(size.x*.38,520*pr)),h=Math.floor(w*.5625),x=Math.floor(12*pr),y=Math.floor(size.y-h-88*pr);cam.aspect=w/h;cam.updateProjectionMatrix();
