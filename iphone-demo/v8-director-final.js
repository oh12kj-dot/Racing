export function createDirector(R){
  let focus=0,shot='TV',banner='',bannerTimer=0,manualHold=0;
  const seen=new Set();
  ...
  function consume(){
    ...
    for(const e of R.events){
      ...
      if(!chosen||(priorities[e.type]||0)>(priorities[chosen.type]||0))chosen=e;
    }
  ...
  function update(dt){
    ...
    const e=consume();
    if(e){const c=e.carId!==null?R.cars[e.carId]:null;if(c)focus=c.id;banner=label(e);bannerTimer=3;if(['RETIREMENT','MECHANICAL','INCIDENT'].includes(e.type))shot='HELI';else if(e.type==='OVERTAKE')shot='CHASE';else if(e.type==='PIT_STOP'||e.type==='PIT_EXIT')shot='TV';else if(e.type==='FASTEST_LAP'||e.type==='FINISH')shot='CHASE';}
    else if(manualHold<=0){const lead=R.getStandings()[0];if(lead)focus=lead.id;const p=(R.race.t|0)%21;shot=p<7?'TV':p<14?'CHASE':'HELI';}
  }
