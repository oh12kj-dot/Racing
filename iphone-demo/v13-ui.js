...
export function createUI(W,R,D,E,C,A,settings,saveSettings){
  ...
  const oldUpdate=base.update;let acc=0;return new Proxy(base,{get(target,prop){if(prop==='update')return(dt,id)=>{oldUpdate(dt,id);acc+=dt;if(acc>.35&&b.classList.contains('on')){acc=0;renderStrategy();}};return Reflect.get(target,prop,target);}});
}
