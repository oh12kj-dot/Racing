export function createSafetyCar(W,R){
  ...
  for(const s of[-1,1]){
    const stripe=add(new T.BoxGeometry(d.width*.16,.05,d.length*.44),black,s*d.width*.31,.74,.12,0,s*.18,0);stripe.rotation.y=s*.12;
    add(new T.BoxGeometry(d.width*.12,.055,.75),white,s*d.width*.27,.76,d.length*.30,0,s*.28,0);
  }

  const bar=new T.Group();bar.position.set(0,1.72,-.10);visual.add(bar);
  const base=new T.Mesh(new T.BoxGeometry(1.62,.10,.28),black);bar.add(base);
  const lights=[];
  for(let i=0;i<6;i++){
    const mat=i%2?amberA:amberB,l=new T.Mesh(new T.BoxGeometry(.22,.14,.24),mat);l.position.set((i-2.5)*.25,.10,0);bar.add(l);lights.push(l);
  }
  for(const s of[-1,1]){
    add(new T.BoxGeometry(.22,.11,.06),amberA,s*.72,.65,d.length*.46);
    add(new T.BoxGeometry(.22,.11,.06),amberB,s*.72,.72,-d.length*.46);
  }
  ...
  function update(){
    ...
    const a=Math.sin(time*15)>0,b=Math.sin(time*15+Math.PI)>0;
    amberA.emissiveIntensity=a?4.8:.22;amberB.emissiveIntensity=b?4.8:.22;
    for(const l of lights)l.scale.y=.88+(l.material.emissiveIntensity>1?.20:0);
  }
  return{update,mesh};
}
